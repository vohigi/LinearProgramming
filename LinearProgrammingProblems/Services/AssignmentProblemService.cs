...
namespace LinearProgrammingProblems.Services
{
    public class AssignmentProblemService
    {
        ...
        public AssignmentProblemServiceResult FindAssignments(int[,] values)
        {
            values = SquareArray(values);
            int[,] costs = new int[values.GetLength(0),values.GetLength(1)];
            for (var i = 0; i < values.GetLength(0); i++)
            {
                for (var j = 0; j < values.GetLength(1); j++)
                {
                    costs[i, j] = values[i, j];
                }
            }
            ...
            if (h != w) costs = SquareArray(costs);
            ...
            var res = new AssignmentProblemServiceResult();
            int[] result = new int[agentsTasks.Length];
            int sum = 0;
            for (int i = 0; i < agentsTasks.Length; i++)
            {
                result[i] = values[i, agentsTasks[i]];
                sum += values[i, agentsTasks[i]];
            }

            res.FinalMatrix = costs;
            res.FunctionValue = sum;
            res.SolutionValues = result;
            return res;
        }

        private int RunStep1(byte[,] masks, bool[] colsCovered, int w, int h)
        {
            if (masks == null)
        ...
        private int RunStep2(int[,] costs, byte[,] masks, bool[] rowsCovered, bool[] colsCovered, int w, int h,
        ...
        private int RunStep3(byte[,] masks, bool[] rowsCovered, bool[] colsCovered, int w, int h, Location[] path,
            Location pathStart)
        {
            if (masks == null)
                throw new ArgumentNullException(nameof(masks));
        ...
        private int RunStep4(int[,] costs, bool[] rowsCovered, bool[] colsCovered, int w, int h)
        {
            if (costs == null)
                throw new ArgumentNullException(nameof(costs));
        ...
        private int[,] SquareArray(int[,] array)
        {
            ...
            var square = new int[loops, loops];
